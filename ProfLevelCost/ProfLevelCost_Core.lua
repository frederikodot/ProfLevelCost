...
local function noteLooksLikeRep(note)
	...
	note = stripColorCodes(note)
	return note:find("Hated")
		or note:find("Hostile")
		or note:find("Unfriendly")
		or note:find("Neutral")
		or note:find("Friendly")
		or note:find("Honored")
		or note:find("Revered")
		or note:find("Exalted")
end
...
local function _PoolAllowed(allowedPools, poolKey)
	if not allowedPools then
		return true
	end
	if type(allowedPools) ~= "table" then
		return true
	end
...
local function _SimAcquire(itemID, qty, skill, inventory, allowedPools, visited, craftListOut)
						...
						if usable then
							...
							for _, rr in ipairs(rec.reagents or {}) do
								...
								if rid and rq > 0 then
									local cCost, cPurch, cMiss =
										_SimAcquire(rid, rq, skill, invSim, allowedPools, cycleGuard, craftListOut)
	...
	if bestPurch and bestCost < math.huge and _CompareCostTriples(bestCost, bestMiss or {}, buyCost, buyMiss or {}) then
		...
		for k, v in pairs(bestInvSim or {}) do
			inventory[k] = v
		end
...
local function getPoolRecipes(poolKey)
	...
	if not ProfLevelCostRecipePools then
		return {}
	end
	...
	local function recipeKey(r)
		...
		if r.spellID then
			return "spell:" .. tostring(r.spellID)
		end
		if r.recipeID then
			return "recipe:" .. tostring(r.recipeID)
		end
...
local function buildVirtualBracketsFromPool(plan)
	...
	if firstChunk and firstChunk <= 0 then
		firstChunk = nil
	end
...
local function applyModeToChance(p, mode)
	...
	if p < 1.0 then
		...
		elseif mode == "max" then
			...
			if pw < 0.01 then
				pw = 0.01
			end -- floor to prevent insane/infinite crafts near gray
			return pw
		end
	end
	...
	if p < 0.01 then
		p = 0.01
	end -- optional: keeps expected finite too (remove if you want true blow-up)
	return p
end
...
local function estimateCraftVariants(bracket, cand)
	...
	if not minC then
		minC = math.huge
		minReason = minReason or "No data"
	end
	if not expC then
		expC = math.huge
		expReason = expReason or "No data"
	end
	if not maxC then
		maxC = math.huge
		maxReason = maxReason or "No data"
	end
...
local function pickCandidateForBracket(bracket, inventory, mode)
	...
	local function betterThan(keyA, keyB)
		...
		if keyA[2] ~= keyB[2] then
			return keyA[2] < keyB[2]
		end
		if keyA[3] ~= keyB[3] then
	...
	for ci, cand in ipairs(bracket.candidates or {}) do
		...
		if considered then
			pricedCost, missingItems, missingQty, missingList =
				evalReagentsCostOnly(cand.reagents or {}, crafts, inventory, skill, allowedPools)
		else
		...
		if br.candidates and #br.candidates > 0 then
			...
			if chosen then
				...
				table.insert(steps, {
					label = string.format(
						"%s - %s",
						(br.label or ("Bracket " .. bi)),
						formatRecipe(chosen.label, chosen.source, chosen.note)
					),
					crafts = crafts,
...
-- Plan helpers
-- =========================================================
local function getPlans()
	return ProfLevelCostPlans or {}
end
...
local function buildSummaryReport(rawPlan)
	...
	table.insert(
		lines,
		"Price source: "
			.. (AuctionatorReady() and "Auctionator (last scanned)" or "Fallback only (Auctionator not ready)")
	)
	table.insert(lines, "Details: " .. detailsLabel())
	table.insert(
		lines,
		"Expand crafted reagents: " .. ((ProfLevelCostDB.expandCraftedReagents ~= false) and "On" or "Off")
	)
	table.insert(lines, "")
	if normPlan._isOptimizer then
		...
		table.insert(
			lines,
			"Include reputation recipes: " .. ((ProfLevelCostDB.includeRepRecipes == true) and "Yes" or "No")
		)
	...
	for i = 1, limit do
		...
		if it.unit then
			table.insert(
				lines,
				string.format("  - %s x%d  @ %s  = %s", name, it.qty, copperToString(it.unit), copperToString(it.cost))
			)
		else
	...
	for _ in pairs(missingPrice) do
	...
	if missingCount > 0 then
		...
		local miss = {}
		for id in pairs(missingPrice) do
			table.insert(miss, id)
		end
		table.sort(miss)
...
local function buildRecipesReport(rawPlan)
	...
	if normPlan._isOptimizer then
		...
		table.insert(
			lines,
			string.format(
				"Show N cheapest options: %d (chosen + %d alternative%s)",
				showN,
				showAlternatives,
				showAlternatives == 1 and "" or "s"
			)
		)
		table.insert(lines, "")
	else
	...
	local function compactMatsFromTotals(totals)
		...
		for id, qty in pairs(totals or {}) do
			qty = tonumber(qty) or 0
			if qty > 0 then
				table.insert(rows, { id = id, qty = qty })
			end
		end
	...
	for _, entry in ipairs(collapsed) do
		if (entry.crafts or 0) <= 0 then
			...
			if comps and #comps > 0 then
				...
				for _, c in ipairs(comps) do
					...
					if (ProfLevelCostDB.includeRepRecipes == true) and not c.usable then
						table.insert(
							lines,
							string.format("     - %s: |cffff3333UNAVAILABLE|r (%s)", name, c.usableReason or "gated")
						)
					else
			...
			local rangeText = (rangeFrom and rangeTo) and string.format("%d-%d", rangeFrom, rangeTo)
				or (entry.bracketLabel or "")
			...
			if entry.option then
				header = header
					.. string.format(
						"  |cff66ff66[%s]|r",
						formatRecipe(entry.option, entry.optionSource, entry.optionNote)
					)
			end
			if entry.craftsMin and entry.craftsExp and entry.craftsMax then
				header = header
					.. string.format(
						"  |cffaaaaaa[range %d/%d/%d]|r",
						entry.craftsMin,
						entry.craftsExp,
						entry.craftsMax
					)
			...
			if compact then
				...
				do
					...
					table.insert(
						lines,
						string.format("   Step cost (chosen, priced items only): %s%s", costStr, suffix)
					)
					...
					if entry.craftedReagents and #entry.craftedReagents > 0 then
						...
						for _, a in ipairs(rows) do
							table.insert(
								...
								string.format(
									"     - %s x%d via %s",
									safeItemName(a.itemID),
									a.qty,
									formatRecipe(a.recipeLabel, a.recipeSource, a.recipeNote)
								)
							)
						end
					end
				...
				for rank = 1, showAlternatives do
					...
					for si, seg in ipairs(segments) do
						...
						table.insert(
							lines,
							string.format(
								"   Alternative #%d%s: |cff66ccff%s|r  (%s)  (est: %s)%s",
								rank,
						...
						if compact then
							...
							for _, r in ipairs(bd) do
								...
								if r.unit then
									line = line
										.. string.format("  @ %s = %s", copperToString(r.unit), copperToString(r.cost))
								else
									line = line .. "  = |cffff3333NO PRICE|r"
								end
								table.insert(lines, line)
							...
							table.insert(
								lines,
								string.format("   Step cost (alternative, priced items only): %s%s", costStr, suffix)
							)

							local missCount = 0
							for _ in pairs(miss or {}) do
							...
							if missCount > 0 then
								table.insert(
									lines,
									string.format(
										"   |cffff3333Alternative missing prices for %d item(s).|r",
										missCount
									)
								)
							end
